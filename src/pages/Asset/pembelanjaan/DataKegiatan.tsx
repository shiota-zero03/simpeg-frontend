...
import { useDeleteDataKegiatan, useGetAllDataKegiatan } from "@/services/asset/asset-pembelanjaan/data-kegiatan";
...
export default function AssetIndex() {
  ...
  const {
    ...
    refetch: refetchData,
  } = useGetAllDataKegiatan(pageIndex + 1, limit, search);
  ...
  const paginatedData: DataProps[] = useMemo(() => {
    if (allData) {
      ...
      return data.response.map((item: DataKegiatanRes) => ({
        id: item.id,
        nama: item.name,
  ...
  const { mutate: mutateDelete } = useDeleteDataKegiatan();
