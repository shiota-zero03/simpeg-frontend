...
import { useDeleteSubDataKegiatan, useGetAllSubDataKegiatan } from "@/services/asset/asset-pembelanjaan/sub-data-kegiatan";
...
export default function AssetIndex() {
  ...
  const {
    ...
    refetch: refetchData,
  } = useGetAllSubDataKegiatan(pageIndex + 1, limit, search);
  ...
  const paginatedData: DataProps[] = useMemo(() => {
    if (allData) {
      ...
      return data.response.map((item: SubDataKegiatanRes) => ({
        id: item.id,
        nama: item.name,
        kode: item.accountBank,
      }));
    } else {
      return [];
    }
  ...
  const { mutate: mutateDelete } = useDeleteSubDataKegiatan();
