...
export default function CreatePegawai() {
  ...
  const { data, isFetching, refetch, error } = useGetDetailAssetService(
    id || "",
  );
  useEffect(() => {
    if (!isFetching && error) {
    ...
    if (data) {
      setFormData({
        ...
        pajak5Tahun: data.data.pajak5Tahun
          ...
          : "",
        pembayaranPajak: data.data.pembayaranPajak
          ? DateYMDFormat(data.data.pembayaranPajak)
          : "",
        nominalBayar: data.data.nominalBayar,
        startServis: data.data.startServis
          ? DateYMDFormat(data.data.startServis)
          : "",
        endServis: data.data.endServis
          ? DateYMDFormat(data.data.endServis)
          : "",
        nominalServis: data.data.nominalServis,
        servicesKe: data.data.servicesKe,
        nomorSurat: data.data.nomorSurat,
        tanggalSurat: DateYMDFormat(data.data.tanggalSurat),
      });
    }
  }, [id, data]);
  ...
  const { mutate: mutatePost } = useUpdateAssetService();
  ...
  return (
    <>
      ...
      <div className="md:p-8 p-4 grid grid-cols-1 gap-8 relative z-10">
        ...
        <TitleCase
          title="Edit Servise/Pajak Aset"
          text="Digunakan Untuk Memperbarui Data Servis/Pajak Aset yang Terbaru"
        />
