...
import { LucideEye, LucidePlusCircle, LucideTrash2, LucideUploadCloud } from "lucide-react";
...
export default function UpdateSPPD() {
  ...
  useEffect(() => {
    if(data) {
      ...
      const partiLeader = dataGet.participants.find(key => key.role === "PEGAWAI");

      const partiMember = dataGet.participants.filter(key => key.role !== "PEGAWAI");
      ...
      setFormData({
          ...
          participantsLeader: {
              ...
              userJabatan: partiLeader?.user.jabatan ? partiLeader?.user.jabatan.nameJob : "",
  ...
  const handleConfirm = () => {
        ...
        budgets: [
          {
            ...
            volTransport: partiLead.budgets ? (partiLead.budgets.volTransport || 0) : 0,
            representatif: partiLead.budgets ? (partiLead.budgets.representatif || 0) : 0,
            volRepresentatif: partiLead.budgets ? (partiLead.budgets.volRepresentatif || 0) : 0,
            dailyAllowance: partiLead.budgets ? (partiLead.budgets.dailyAllowance || 0) : 0,
            volDailyAllowance: partiLead.budgets ? (partiLead.budgets.volDailyAllowance || 0) : 0,
            bankAccount: partiLead.bankAccount,
        }]
      });
    }
        ...
        budgets: [
          {
            ...
            volRepresentatif: item.budgets ? (item.budgets.volRepresentatif || 0) : 0,
            dailyAllowance: item.budgets ? (item.budgets.dailyAllowance || 0) : 0,
            volDailyAllowance: item.budgets ? (item.budgets.volDailyAllowance || 0) : 0,
            bankAccount: item.bankAccount,
        }]
      });
    });
    ...
    try {
      mutatePost({
        id: id as string, formData: formToSendData
      }, {
        onSuccess: () => {
