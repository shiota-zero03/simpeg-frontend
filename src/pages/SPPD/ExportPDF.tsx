...
export default function ExportIKP() {
  ...
  DATA_DETAIL?.participants.map((item) => {
    totalAnggaran +=
      ...
      Number(
        (item.budgets[0].volTransport || 0) * (item.budgets[0].transport || 0),
      ) +
      Number(
      ...
      {DATA_DETAIL && (
        <div className="w-full p-8">
          <div className="text-center">
            <h1 className="font-bold">
              {DATA_DETAIL.type === "PERJALANAN_BIASA"
                ? "DAFTAR TANDA TERIMA"
                : "DAFTAR TANDA TERIMA UANG TRANSPORT"}
            </h1>
          </div>
          <br />
          <table>
          ...
          <table className="w-full">
            <thead>
              <tr>
                <th className="p-1 border border-black bg-blue-100">No</th>
                <th className="p-1 border border-black bg-blue-100">
                  Nama / NIP
                </th>
                <th className="p-1 border border-black bg-blue-100">JABATAN</th>
                <th className="p-1 border border-black bg-blue-100">
                  NO. REK BJB
                </th>
                <th className="p-1 border border-black bg-blue-100">URAIAN</th>
            ...
            <tbody>
              {DATA_DETAIL.participants.map((item, index) => (
                <React.Fragment key={index}>
                  <tr>
                    <td
                      className="text-center p-1 border border-black align-top text-[10pt]"
                      rowSpan={3}
                    >
                      {index + 1}
                    </td>
                    <td
                      className="p-1 border border-black align-top text-[10pt]"
                      rowSpan={3}
                    >
                      {item.user.name} / <br />
                      {item.user.nip}
                    ...
                    <td
                      ...
                      rowSpan={3}
                    >
                      {item.user.jabatan ? item.user.jabatan.nameJob : "-"}
                    </td>
                    <td
                      className="p-1 border border-black align-top text-[10pt]"
                      rowSpan={3}
                    ...
                    <td className="p-1 border border-black align-top text-[10pt]">
                      Uang Harian
                    </td>
                  ...
                  <tr>
                    ...
                    <td className="p-1 border border-black align-top text-[10pt] text-right">
                      {item.budgets[0]
                        ? Number(item.budgets[0].transport || 0).toLocaleString(
                            "id-ID",
                          )
                        : 0}
                    </td>
