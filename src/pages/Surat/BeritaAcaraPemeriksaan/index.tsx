...
export default function SuratPemeriksaan() {
  ...
  const {
    ...
    refetch: refetchData,
  } = useGetAllBeritaAcaraPemeriksaan(pageIndex + 1, limit, search, searchNo);
  ...
  const paginatedData: DataProps[] = useMemo(() => {
    if (allData) {
      ...
      return data.response.map((item: BeritaAcaraPemeriksaanRes) => ({
        id: item.id,
  ...
  const { mutate: mutateDelete } = useDeleteBeritaAcaraPemeriksaan();
  ...
  return (
    <>
      <BreadcrumbAdmin location="/Berita-Acara-Pemeriksaan" />
      <DeleteModal
        isOpen={isOpenDelete}
        onClose={onCloseDelete}
      ...
      <div className="md:p-8 p-4 grid grid-cols-1 gap-8">
        <div className="flex items-center justify-between gap-2 md:flex-row flex-col">
          <TitleCase
            ...
            text="Berikut ini Mengelola Daftar Berita Acara Pemeriksaan"
          />
          <Button
            onPress={onOpenKop}
