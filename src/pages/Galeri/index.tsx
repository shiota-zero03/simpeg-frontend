...
import { GaleriRes } from "@/interface/responses/galeri.interface";
...
export default function News() {
  ...
  const {
    ...
    refetch: refetchData,
  } = useGetAllGaleri(pageIndex + 1, limit, search);
  ...
  const paginatedData: BeritaDataProps[] = useMemo(() => {
    if (allData) {
      ...
      return data.response.map((item: GaleriRes) => ({
        id: item.id,
        title: item.title,
        type: item.type,
        images: item.images,
  ...
  const columns: ColumnDef<BeritaDataProps>[] = [
      ...
      cell: (info) => {
        ...
        return picture ? (
          <div className="flex items-center justify-center">
            <img
              src={picture}
              alt="galeri-dokumentasi-picture"
              width={80}
    ...
    },
    {
      ...
      header: "Tipe",
      cell: (info) =>
        (info.getValue() as string) === "IMAGE" ? "Gambar" : "Video",
      // meta: { align: "center" },
    },
    {
      accessorKey: "video",
      ...
      cell: (info) => {
        ...
        return link ? (
          <Link to={link} target="__blank" className="text-blue-500 underline">
            {link}
          </Link>
        ) : (
          "-"
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2 justify-center">
            <Button
              onPress={() => navigate(`/galeri-dokumentasi/edit-data/${id}`)}
              isIconOnly
              radius="sm"
              size="sm"
  ...
  const { mutate: mutateDelete } = useDeleteGaleri();
  ...
  return (
    <>
      <BreadcrumbAdmin location="/Galeri-Dokumentasi" />
      <DeleteModal
        isOpen={isOpenDelete}
        onClose={onCloseDelete}
      ...
      <div className="md:p-8 p-4 grid grid-cols-1 gap-8">
        <TitleCase
          title="Galeri"
          text="Berikut ini menampilkan Daftar Galeri yang sudah tersimpan"
        />
              ...
              <div className="flex sm:flex-row flex-col justify-between gap-2 sm:items-end">
                <div className="flex sm:flex-row flex-col gap-2 items-end w-full">
                  <Input
                    ...
                    size="sm"
                    variant="bordered"
                    placeholder="Cari judul galeri disini"
                    startContent={
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  <Button
                    onPress={() => navigate("/galeri-dokumentasi/tambah-data")}
                    variant="solid"
                    radius="sm"
                    size="sm"
