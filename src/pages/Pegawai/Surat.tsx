...
import { useDeleteSuratPegawai, useGetAllSuratPegawai } from "@/services/pegawai";
  ...
  const paginatedData: DataProps[] = useMemo(() => {
    if (allData) {
      ...
      return data.response.map((item: SuratPegawaiRes) => ({
        ...
        tipe: item.typeForm === "CUTI" ? "Cuti" : (
            item.typeForm === "KENAIKAN_GAJI" ? "Kenaikan Gaji" : "Kenaikan Pangkat"
        )
      }));
    } else {
      return [];
