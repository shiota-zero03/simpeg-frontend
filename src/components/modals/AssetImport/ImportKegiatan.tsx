...
import { useCreateDataKegiatan } from "@/services/asset/asset-pembelanjaan/data-kegiatan";
...
const ImportKegiatan = ({ isOpen, onClose, handleClose }: props) => {
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [fileExcel, setFileExcel] = useState<File | null>(null);
  const [isDownloading, setIsDownloading] = useState(false);

  const { mutate: mutatePost } = useCreateDataKegiatan();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const item of data) {
        ...
        const formToSend: StoreDataKegiatan = {
          name,
  ...
  const handleExportTemplate = async () => {
    ...
    worksheet.columns = [
      ...
      { header: "Nama Kegiatan", key: "name", width: 30 },
    ...
    saveAs(new Blob([buffer]), "template_data_kegiatan.xlsx");
    ...
    <Modal isOpen={isOpen} backdrop="blur" hideCloseButton size="3xl">
      <ModalContent>
        <ModalHeader className="flex items-center justify-between">
          <span className="text-base font-semibold">Import Data Kegiatan</span>
          <LuX
