...
export function AppSidebar() {
    ...
    const isMenuActive = (items: { key: string; subMenu?: any[] }[], allowedSubMenus: string[]): boolean => {
  ...
  return (
    <Sidebar collapsible="icon" className="!border-none !outline-none !ring-0 overflow-hidden !z-50 bg-primary">
                                    ...
                                    <CollapsibleTrigger asChild>
                                        <Link to={itemM.link} 
                                            className={`font-light text-xs px-4 hover:font-medium hover:text-secondary rounded-none ${
                                                isActive ? 'border-s-4 border-secondary bg-[#FFF1001A] text-secondary' : 
                                  ...
                                  <CollapsibleContent>
                                    <SidebarMenuSub className="!border-none">
                                      {itemM.subMenu.map(subItem => {
                                        if(allowedSubMenus.includes(subItem.key)) {
                                          return (
                                            <SidebarMenuSubItem key={subItem.key} className="py-1">
                                              <Link to={subItem.link} className={`font-light px-2 hover:font-medium hover:text-secondary rounded-none duration-300 flex items-center gap-2 text-xs ${location.pathname.includes(subItem.key) ? 'text-secondary' : 'text-white'}`}>
                                                <FaCircle size={6} />
